...
const Accounts = () => {
    ...
    useEffect(() => {
        if (!isStatusInclude(accountData?.status)) return;
        const { rollNumber, contactNumber, name } = searchQuery;
        setFilteredAccountData(accountData?.data);

        // if (rollNumber || contactNumber || name) {
        //     setFilteredAccountData((prevData) => {
        //         return prevData?.filter(({ rollNumber, contactNumber, studentName, studentSurName }) => {
        //             const isSearchIncludes = (query) => {
        //                 return query?.toLowerCase().includes(searchQuery["rollNumber"].toLowerCase() || searchQuery["contactNumber"] || searchQuery["name"]);
        //             };

        //             return isSearchIncludes(rollNumber) || isSearchIncludes(contactNumber) || isSearchIncludes(`${studentName} ${studentSurName}`);
        //         });
        //     });
        // }
        if (rollNumber) {
            setFilteredAccountData((prevData) => {
                return prevData?.filter(({ rollNumber }) => {
                    const isSearchIncludes = (query) => {
                        return query?.toLowerCase().includes(searchQuery["rollNumber"].toLowerCase());
                    };
                    return isSearchIncludes(rollNumber);
                });
            });
        }
        if (contactNumber) {
            setFilteredAccountData((prevData) => {
                return prevData?.filter(({ contactNumber }) => {
                    const isSearchIncludes = (query) => {
                        return query?.toLowerCase().includes(searchQuery["contactNumber"]);
                    };
                    return isSearchIncludes(contactNumber);
                });
            });
        }
        if (name) {
            setFilteredAccountData((prevData) => {
                return prevData?.filter(({ studentName, studentSurName }) => {
                    const isSearchIncludes = (query) => {
                        return query?.toLowerCase().includes(searchQuery["name"]);
                    };
                    return isSearchIncludes(`${studentName} ${studentSurName}`);
                });
            });
        }
    ...
    }, [accountData, accountFilterObj, searchQuery, byFees, byPaymentMethod, percentageTo, percentageFrom, feesReportFrom, feesReportTo, chequeNumber]);
    ...
    return (
        <>
            <div className=" overflow-x-hidden overflow-y-auto pb-5" style={{ overflowY: "auto", height: "calc(100vh - 80px)" }}>
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div ref={wrapperRef} style={{ zIndex: "5" }}>
                            <button className="border-0 bg-white" onClick={handleFilterButtonClick}>
                                <img src={filter} alt="filter icon" loading="eager" />
                            </button>
                ...
                <Row className="mt-3">
                    <Col lg={4}>
                        <AccountSearchInput name="rollNumber" handleSearchQuery={handleSearchQuery} placeholder="Search by Roll No..." valueObj={searchQuery} />
                    </Col>
                    <Col lg={4}>
