...
const ExamReport = () => {
    ...
    useEffect(() => {
        if (!isStatusInclude(testNumberResponse?.status)) return;
    ...
    const handleStudentReportSubmit = async (data) => {
        ...
        if (isDateValid) {
            ...
            toast.error("select a valid standard or test number");
        }
    };
            ...
            <Container fluid className="mt-3" style={{ height: "calc(100vh - 216px)", overflow: "auto" }}>
                <Form onSubmit={handleSubmit(handleStudentReportSubmit)}>
                    <Row className="mb-2">
                        ...
                        <Col sm={12} md={4} style={{ height: "118px" }}>
                            <Label name="date" title="Date" form={form} classNameLabel={"default_label"} />
                            <Controller
                                control={control}
                                name="date"
