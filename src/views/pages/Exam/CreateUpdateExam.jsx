...
const CreateUpdateExam = () => {
    ...
    useErrorCatcher({ error: updateError, isError: isUpdateError });

    useEffect(() => {
        if (!isStatusInclude(testNumber?.status)) return;
    ...
    useEffect(() => {
        if (!isStatusInclude(stdAndSubjectResponse?.status)) return;
    ...
    const onSubmit = async (data) => {
        ...
        const response = isCreateMode ? await createExam(examData) : await updateExam({ examId, examData });
        if (response?.data?.status === (201 || 202)) {
    ...
    const handleSubjectsOptionsAsPerStd = useCallback(
        (standardId) => {
            const subObjAsPerStd =
                isValidArray(subjectOptionsForAllStd) &&
                subjectOptionsForAllStd?.find((item) => {
                ...
                <Form onSubmit={handleSubmit(onSubmit)}>
                    <div className="position-relative w-100">
                        <div className="w-100">
                            <Row className="mb-2">
                                <Col sm={12} lg={6}>
                                    <Label name="Standard" title="Standard" form={form} classNameLabel={"default_label"} />
                                    <Controller
                                        control={control}
                                        name={`standardId`}
                                ...
                                <Col lg={6}>
                                    <Label name="date" title="Date Selection" form={form} classNameLabel={"default_label"} />
                                    <Controller
                                        name="date"
                                        control={control}
                    ...
                    <div className="d-flex justify-content-center">
                        <button disabled={isFetching || isFetchingTestNumber} type="submit" className="staff-button p-2">
