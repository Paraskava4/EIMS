...
const Noticeboard = () => {
    ...
    useEffect(() => {
        ...
        setNoticeData((prevData) => {
            ...
            const sortedNoticeData = [...prevData]?.sort((a, b) => {
...
export const SingleNotice = (props) => {
    const { date, details, postedByName, timeDifference, title } = props?.data;
    const { handleEdit, handleDelete, index, className } = props;
    const colors = ["#026B81", "#768102", "#1B0281", "#810209", "#C8309D"];

    return (
        <div className="Notice_wrapper">
            <p className="text-start">
                <span className="date_text text-start fs-6 py-1" style={{ background: colors[index % colors.length] }}>
