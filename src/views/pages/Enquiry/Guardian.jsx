...
const GuardianDetail = (props) => {
    ...
    const { fields, append: appendSibling, remove: removeSibling } = useFieldArray({ control, name: "siblings" });
    ...
    const formatSubjects = (subjectsArray) => {
        if (!isValidArray(subjectsArray)) return [];
        return subjectsArray?.map(({ value }) => ({ subjectId: value }));
    };
    ...
    const onSubmit = async (data) => {
        personalData = {
            ...
            dateOfBirth: format(new Date(personalData.dateOfBirth), "dd/MM/yyyy"),
        };
        educationData = { ...educationData, subjects: formatSubjects(educationData?.subjects) };
                                    ...
                                    <Row className="w-100 mt-2">
                                        <Col sm={12} lg={6}>
                                            <InputItem
                                                ...
                                                form={form}
                                                classNameLabel={"default_label m-0 p-0"}
                                                className={"form__input border-0 form-control-lg"}
                                            />
                                        </Col>
                                        <Col sm={12} lg={6}>
                                            <div className="p-0 m-0">
                                                <Form.Label className="w-100 default_label text-start">
                                                    Study here
                                                    <Controller
                                                        control={control}
...
export const CustomFormSelect = ({ form, options, label, name }) => {
    const { control } = form;
    return (
        <ModifiedSelect
            name={name}
